...
public class DataCollect {
	...
	// private Vector<String > totalDataList = new Vector<String >();

	/* 控制地址控制是否上升沿导出csv文件 */
	private Vector<Integer> m_bHistorySaveReset = new Vector<Integer>();
	// private Vector<Integer > m_nSaveIndex = new Vector<Integer >();
	...
	private void collectData() {
		...
		// Log.d("SKScene",
		// "DataCollent allTime:"+(System.currentTimeMillis()-sTiem));
	}
	...
	private void init() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (mCollectDbObj != null) {
				/* 取数据库记录数 */
				Cursor cus = mCollectDbObj.getDatabaseBySql(
						"select count(id) as count from dataCollect" + i, null);
				if (null != cus) {
					if (cus.moveToNext()) {
		...
		/* 采集的数目 */
		int nGrouSize = CollectDataInfo.getInstance().getmHistoryInfoList()
				.size();
		if (nGrouSize == 1) {
			/* 一组的时候30万最多 */
		...
		// for test , "1970-01-02 01:38:46", "1970-01-02 15:04:05"
		// Vector<Integer> nChannelList = new Vector<Integer>();
		// for(int i = 0; i < 5; i++)
		// {
		// nChannelList.add(i);
		// }
		...
		// DataCollect.getInstance().getGroupCollectData(1, 100, nChannelList,
		// null);
		// Vector<Vector<String > > list =
		// DataCollect.getInstance().getGroupCollectDataByTime(1, 100,
		// nChannelList, "1970-01-02 01:38:46", "1970-01-03 15:04:05", null);
		// if(list != null && !list.isEmpty())
		// {
		// System.out.println("Channel data size is:" + list.size());
		// System.out.println("Channel size is:" + list.get(0).size());
		// }
	}
	...
	private void refreshAddrCtlDataCollect() {
		int nGroupSize = CollectDataInfo.getInstance().getmHistoryInfoList()
				.size();
		for (int i = 0; i < nGroupSize; i++) {
			if (CollectDataInfo.getInstance().getmHistoryInfoList().get(i)
					.isbAutoSave()) {
				long nCurrTime = System.currentTimeMillis();
				int nIntervalTime = CollectDataInfo.getInstance()
			...
			// else if(nCtlValue == 2)
			// {
			// SkGlobalBackThread.EditDataCollectProp mEditData =
			// SkGlobalBackThread.getInstance().new EditDataCollectProp();
			// mEditData.nGroupId = nGroupId;
			// mEditData.nRecordTimeLen = nSaveTimeLen;
			//
			// /*发送消息*/
			// SkGlobalBackThread.getInstance().getGlobalBackHandler().obtainMessage(MODULE.DELETE_HISTORY_FROM_DATABASE,
			// mEditData).sendToTarget();
			// }
		}
	}
	...
	private void dealPlcNotic(int eCollectType, int nGroupId,
		...
		// System.err.println("nDataList"+nDataList);
		if (null == nDataList || nDataList.nDataList.isEmpty())
			return;
		// System.err.println("m_noticCallBackList"+m_noticCallBackList);
		...
		for (int i = 0; i < nCallbackSize; i++) {
			if (m_noticCallBackList.get(i).eCollectType == eCollectType) {
					...
					;
				{
					...
					/* 清除老数据 */
					// m_noticCallBackList.get(i).nDataList.get(nIndex).nMillisTime
					// = nDataList.nMillisTime;
					// m_noticCallBackList.get(i).nDataList.get(nIndex).bCollectFull
					// = nDataList.bCollectFull;
					//
					// m_noticCallBackList.get(i).nDataList.get(nIndex).nDataList.clear();
					// int nNewSize = nDataList.nDataList.size();
					// for(int k = 0; k < nNewSize; k ++)
					// {
					// m_noticCallBackList.get(i).nDataList.get(nIndex).nDataList.add(nDataList.nDataList.get(k));
					// }
	 ...
	 * @return
	 */
	private boolean isBitOnAddrCtlSamp(int eCollectType, int index) {
		int nSize = 0;
		boolean bAddrCtlSamp = false;
		AddrProp mCtlSampAddr = null;
		boolean result=true;
		...
		if (nRealTimeCollect == eCollectType) {
			...
			/* 是否地址控制采样 */
			bAddrCtlSamp = CollectDataInfo.getInstance().getmRealTimeInfoList()
					.get(index).isbAddrCtlSamp();

			/* 控制地址 */
			mCtlSampAddr = CollectDataInfo.getInstance().getmRealTimeInfoList()
					.get(index).getnCtlSampAddrId();
			...
			/* 是否地址控制采样 */
			bAddrCtlSamp = CollectDataInfo.getInstance().getmHistoryInfoList()
					.get(index).isbAddrCtlSamp();

			/* 控制地址 */
			mCtlSampAddr = CollectDataInfo.getInstance().getmHistoryInfoList()
					.get(index).getnCtlSampAddrId();
		}

		/* 是否地址控制采样 */
		if (bAddrCtlSamp) {
			if (null == mCtlSampAddr){
				return false;
			}
			...
			/* get data from plc */
			boolean bSuccess = PlcRegCmnStcTools.getRegIntData(mCtlSampAddr,
					tmpDataList, mSendData);
			if (!bSuccess || tmpDataList.isEmpty())
				result=false;

			if (1 != tmpDataList.elementAt(0)) {
				result=false;
			}
			
			//用于上升沿采样off->on
			if(result==true){
				if(!CollectDataInfo.getInstance().getmHistoryInfoList().get(index).isbCollect()){
					CollectDataInfo.getInstance().getmHistoryInfoList().get(index).setbCollect(true);
				}
			}else {
				CollectDataInfo.getInstance().getmHistoryInfoList().get(index).setbCollect(false);
			}
		}
	...
	private void msgAddNoticProp(DataCollectCallBack mCallbackInfo) {
		...
		Log.d("SKScene", "size:"+m_noticCallBackList.size());
	}
	...
	private void msgDestoryCallback(IPlcNoticCallBack mCallBack) {
		...
		if (nIndex >= 0 && nIndex < nSize) {
			Log.d("SKScene", "remove id:"+nIndex);
			m_noticCallBackList.removeElementAt(nIndex);
		}
	}
	...
	private synchronized boolean readDataFromPlc(int eCollectType, int index,
		...
		if (nRealTimeCollect == eCollectType) {
			...
			for (int i = 0; i < addrSize; i++) {
				...
				/* 如果是字符串 */
				if (DATA_TYPE.ASCII_STRING == eDataType) {
					...
					if (type==SAMP_TYPE.BIT_ON_SAMP) {
						//位ON时控制采样,直接读取
						AddrProp mSampAddr = CollectDataInfo.getInstance().getmRealTimeInfoList().get(index).getmCollectAddrList().get(i).mAddrPro;
						sTmpStr=getSValue(mSampAddr, eDataType);
						
					}else {
						//其他方式，采用通知回调
						CallbackItem item=CollectDataInfo.getInstance()
								.getmRealTimeInfoList().get(index).getmCallbackItems().get(i);
						if (item.hasCallback()) {
							//注册回调，并且已经回调
			...
			for (int i = 0; i < addrSize; i++) {
				...
				if (DATA_TYPE.ASCII_STRING == eDataType) {
					...
					if (type==SAMP_TYPE.BIT_ON_SAMP) {
						//位ON时控制采样,直接读取
						AddrProp mSampAddr = CollectDataInfo.getInstance().getmHistoryInfoList().get(index).getmCollectAddrList().get(i).mAddrPro;
						sTmpStr=getSValue(mSampAddr, eDataType);
					}else {
						//其他方式，采用通知回调
						CallbackItem item=CollectDataInfo.getInstance()
								.getmHistoryInfoList().get(index).getmCallbackItems().get(i);
						if (item.hasCallback()) {
							//注册，并且回调
							sTmpStr=item.getsValue();
						}else{
	...
	private double getNValue(AddrProp mSampAddr,DATA_TYPE type){
		...
		boolean bSuccess = PlcRegCmnStcTools.getRegDoubleData(mSampAddr, tmpDoubleList, mSendData);
	...
	private String getSValue(AddrProp mSampAddr,DATA_TYPE type){
		...
		if(bSuccess && !tmpByteList.isEmpty()){
			value=getAsciid(tmpByteList);
		}
		return value;
	}
	
	/**
	 * byte转成asciid UNICODe编码
	 */
	private String getAsciid(Vector<Byte> temp){
		String sTmpStr="";
		...
		try {
			...
			Log.e("getmHistoryInfoList","readDataFromPlc data collect value to string failed");
			e.printStackTrace();
		}
		return sTmpStr;
	}
	...
	private void collectRealtimeData() {
		...
		for (int i = 0; i < size; i++) {
			...
			case FIXED_TIME_SAMP: // 固定时间采样
			{
				...
				/* 是否地址控制采样 */
				if (isBitOnAddrCtlSamp(nRealTimeCollect, i) == false) {
			...
			case BIT_ON_SAMP: {
				...
				/* 上升沿采样*/
				isBitOnAddrCtlSamp(nHistoryCollect, i);
				
				if(!CollectDataInfo.getInstance().getmHistoryInfoList().get(i).isbCollect()){
...
//						.setbOnValue(true);
				...
				/* 是否自动复位 */
				boolean bAutoReset = CollectDataInfo.getInstance()
						.getmRealTimeInfoList().get(i).isbAutoReset();
	...
	private void collectHistoryData() {
		...
		for (int i = 0; i < size; i++) {
			...
			case FIXED_TIME_SAMP: // 固定时间采样
			{
				...
				/* 是否地址控制采样 */
				if (isBitOnAddrCtlSamp(nHistoryCollect, i) == false) {
...
//						.setbOnValue(true);
				...
				/* 是否自动复位 */
				boolean bAutoReset = CollectDataInfo.getInstance()
						.getmHistoryInfoList().get(i).isbAutoReset();
				...
				if (bAutoReset) {
					...
					boolean bSuccess = PlcRegCmnStcTools.setRegIntData(
							CollectDataInfo.getInstance().getmHistoryInfoList()
									.get(i).getnCtlSampAddrId(), tmpDataList,
							mSendData);
...
//							.setbOnValue(false);
	 ...
	 */
	public void addNoticProp(int eCollectType, Vector<Integer> nGroupIdList,
		...
		getCollectNoticHandler().obtainMessage(MODULE.ADD_CALL_BACK,
				mCallbackInfo).sendToTarget();
	}
	...
	public void destoryDataCollectCallback(IPlcNoticCallBack mCallBack) {
		...
		getCollectNoticHandler().obtainMessage(
				MODULE.DATA_COLLECT_DESTORY_NOTIC, mCallBack).sendToTarget();
	}
	...
	public boolean writeDataToDatabase(EditDataCollectProp mEditProp) {
		if (null == mEditProp || null == mEditProp.nHistoryDataList)
			return false;
		if (mEditProp.nHistoryDataList.isEmpty())
			return false;
		...
		try {
			...
			for (int i = 0; i < nGroupSize; i++) {
				...
				for (int j = 0; j < nDataSize; j++) {
					...
					/* for test */
					// Date mStartDate = new Date(nCurrMillis*1000);
					// SimpleDateFormat formatStr = new
					// SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					// String sStartDate = formatStr.format(mStartDate);
					// m_tmpValues.put("mDateTime", sStartDate);
					/* test end */

					// m_tmpValues.put("nAddrNum", nAddrSize);
					m_tmpValues.put("nMillis", nCurrMillis);
					for (int k = 0; k < nAddrSize; k++) {
						m_tmpValues.put("data" + k, mEditProp.nHistoryDataList
								.get(i).get(j).nDataList.get(k));
					}
					ss = mCollectDbObj.insertData("dataCollect" + i,
							m_tmpValues);
				}
				...
				if (nDataSize > 0) {
					...
					mLastSaveList.set(i,
							mEditProp.nHistoryDataList.get(i)
									.get(nDataSize - 1));
				}
	...
	public boolean writeGroupDataToFile(EditDataCollectProp mEditProp) {
		...
		Log.d("SKScene", "writeGroupDataToFile......");
		if (null == mEditProp){
			//操作失败
			return false;
		}
		...
		/* 判断nDataGroupId是否超限 */
		int nGroupRecipeSize = CollectDataInfo.getInstance()
		...
		if (mEditProp.mMedia!=null) {
			...
			//使用采集默认路径
			eSaveMedia = CollectDataInfo.getInstance()
					.getmHistoryInfoList().get(mEditProp.nGroupId).geteSaveMedium();
		}
		...
		case INSIDE_DISH: {
			...
			sFilePath = "/data/data/com.android.Samkoonhmi/AK_History/data"+mEditProp.nGroupId;
			break;
		}
		case U_DISH: {
		...
		try {
			...
			for (int i = 1; i <= nAddrSize; i++) {
				String name=CollectDataInfo.getInstance().getmHistoryInfoList()
						.get(mEditProp.nGroupId).getmCollectAddrList().get(i-1).sName;
			...
			/* 计算最近时间 */
			String sWhereStr = " order by id";
			if (mEditProp.nRecordTimeLen > 0) {
				/* 取当前时间 */
				long nCurrMillis = System.currentTimeMillis();
				nCurrMillis = (nCurrMillis - mEditProp.nRecordTimeLen * 3600 * 1000);
				sWhereStr = " where nMillis >= " + nCurrMillis;
			}

			Cursor result = mCollectDbObj.getDatabaseBySql(
					"select * from dataCollect" + mEditProp.nGroupId
							+ sWhereStr, null);
			...
			/* 取得数据 */
			String sLineDataStr = "";
			int nAddrNum = result.getColumnCount() - 2;
			while (result.moveToNext()) {
				long nCurrMillis = result.getLong(result
						.getColumnIndex("nMillis"));
				...
				if (sDateStr != null) {
					try {
						...
						if (sDateTime != null && sDateTime.length >= 2) {
							// sLineDataStr += sDateTime[0] + ",";
							// sLineDataStr += sDateTime[1] + ",";
				...
				for (int i = 0; i < nAddrNum; i++) {
					// sLineDataStr+=result.getString(result.getColumnIndex("data"
					// + i));
					// sLineDataStr += ",";
	...
	public synchronized Vector<Vector<String>> getGroupCollectData(
		...
		if (nGetNum <= 1) {
			...
			nMillesDx = ((float) (nLastMilles - nFirstMilles)) / (nGetNum - 1);
		}
		...
		if (null != result) {
			...
			for (int i = 0; i < nAddrLen; i++) {
				nAddrId = nChannelList.get(i);
				nMinColunIndex.add(result.getColumnIndex("nMinData" + nAddrId));
				nMaxColunIndex.add(result.getColumnIndex("nMaxData" + nAddrId));
			}
			...
			while (result.moveToNext()) {
				...
				if (m_noticDataList.isEmpty()) {
					for (int i = 0; i < nAddrLen; i++) {
						double nMin = result.getDouble(nMinColunIndex.get(i));
					...
					for (int i = 0; i < nAddrLen; i++) {
						double nMin = result.getDouble(nMinColunIndex.get(i));
						double nMax = result.getDouble(nMaxColunIndex.get(i));
						// double nAvg =
	 ...
	 * @param nGroupId
	 * @param nGetNum
	 * @param sDateTimeBegin
	 *            起始时间,格式：“2012-5-6 8:23:42" 中间一定用空格隔开
	 * @param sDateTimeEnd
	 *            结束时间,格式：“2012-5-6 8:23:42" 中间一定用空格隔开
	 * @param sFirstTime
	 *            开始时间，传出参数
	 * @return
	 */
	public synchronized Vector<Vector<String>> getGroupCollectDataByTime(
		...
		String sDateTimeQuery = " where nMillis between " + nFirstMilles
				+ " and " + nLastMilles;
		// String sDateTimeQuery = " and mDateTime >= '" + sDateTimeBegin + "'"
		// + " and mDateTime <= " + "'" + sDateTimeEnd + "'" ;
	...
	public synchronized Vector<HistoryDataProp> getHistoryDataByTime(
		...
		Vector<HistoryDataProp> noticDataList = getHistoryDataBySQL(nGroupId,nChannelList,sQueryStr);
		
		System.out.println("getHistoryDataByTime need time :"
				+ (System.currentTimeMillis() - nCurrMillis) + "ms");
	...
	public Vector<HistoryDataProp> getHistoryDataByCoditin(int nGroupId
			, int top, int nGetNum, Vector<Integer> nChannelList, String sQueryElement, String SearchCondition) {
		// TODO Auto-generated method stub
		
		long nCurrMillis = System.currentTimeMillis();
				
		String sQueryStr = "select * from dataCollect"+nGroupId+
			" where "+sQueryElement+SearchCondition+" limit " + (top - 1) + "," + nGetNum;
	...
	public int getDataCount(int gid, long start, long end) {
		...
		// Log.d("SKScene", "time:"+(System.currentTimeMillis()-s));
		return result;
	}
	 ...
	 * @return
	 */
	public int getDataCount(int nGroupId, 
		...
		String sql = "";
		sql = "select count(*) as num From dataCollect"+nGroupId+
				" where "+sQueryElement+SearchCondition;
	...
	public void msgClearAllHistory(ArrayList<Integer> mList) {
		...
		for (int j = 0; j < mList.size(); j++) {
			int nCallbackSize = m_noticCallBackList.size();
			for (int i = 0; i < nCallbackSize; i++) {
				m_noticCallBackList.get(i).mCallbackInterface.noticDelDatabase(mList.get(j));
			}
		}

		mHandler.obtainMessage(MODULE.DATA_COLLECT_CLEAR, mGroupId).sendToTarget();
		/* 发消息清除缓存 */
		//mHandler.sendEmptyMessage(MODULE.DATA_COLLECT_CLEAR);
	}
