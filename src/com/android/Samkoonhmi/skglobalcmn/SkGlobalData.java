...
public class SkGlobalData {
	...
	public static void createMenuTable(){
		 ...
		 * nItemHeigh-画面的高度
		 * nFontSize-画面的大小
		 * nPageId-画面所在页码，从1开始计数
		 * nPagePos-画面所在页面的位置，从0开始计数；
		 */
	...
	public synchronized static SKDataBaseInterface getRecipeDatabase()
	{
		if(null == m_recipeDatabase)
		{
			...
			m_recipeDatabase.openDatabase("/data/data/com.android.Samkoonhmi/formula/recipe.dat");
		}
	...
	public synchronized static SKDataBaseInterface getDataCollectDatabase() {
		...
		if(null == m_dataCollectDatabase)
		{
			...
			else
			{
				...
				//int nHistorySize = CollectDataInfo.getInstance().getmHistoryInfoList().size();
				
				ArrayList<Integer> list=DataCollectBiz.getHisAddrSize();
				int nHistorySize=DataCollectBiz.getHistCollentCount();
				Log.d("SKScene", "..................nHistorySize:"+nHistorySize);
				for(int i = 0; i < nHistorySize; i++)
				{
					...
					sTmpSqlStr += (sDataColum + ");");
					bSuccess &= m_dataCollectDatabase.createDatabase(sDatabasePath, sTmpSqlStr);
				}
				...
				bSuccess &= m_dataCollectDatabase.createDatabase(sDatabasePath, sTableName);
				if(bSuccess)
				{
					...
					{
						mTmpValues.clear();
						//String sName = CollectDataInfo.getInstance().getmHistoryInfoList().get(i).getsName();
						String sName="";
						if (mList.size()>i) {
							sName=mList.get(i);
						}
						...
						m_dataCollectDatabase.insertData("collectGroupName", mTmpValues);
					}
				}
	...
	public synchronized static SKDataBaseInterface getAlarmDatabase(){
		...
		if (mAlarmDatabase==null) {
			...
			if (mFile.exists()) {
				...
				try {
					File dir=new File("/data/data/com.android.Samkoonhmi/alarm");
					if(!dir.exists()){
						dir.mkdir();
					}
					mFile.createNewFile();
				...
				boolean bSuccess = true;
				/**
				 *报警登录，数据存储表 dataAlarm
				 * nGroupId-报警组id nAlarmIndex-组里面每个报警id
				 * nDateTime-报警触发时间和日期
				 * nClearDT-报警消除时间和日期
				...
				bSuccess &= mAlarmDatabase.createDatabase(sDatabasePath, alarmSql);
				...
				bSuccess &= mAlarmDatabase.createDatabase(sDatabasePath, alarmMsgSql);
				...
				bSuccess &= mAlarmDatabase.createDatabase(sDatabasePath, alarmlanSql);
