...
public class CRC16 {
	...
	private static final byte[] auchCRCLo = { 0x00, (byte) 0xC0, (byte) 0xC1,
	...
	/*
	 * 计算CRC16校验码，结果存入result中
	 */
	public static void CheckCRC16(byte[] value, int len, byte[] result) {
		byte tCRCHi = (byte) 0xFF; /* 高CRC字节初始化 */
		byte tCRCLo = (byte) 0xFF; /* 低CRC 字节初始化 */
		int uIndex; /* CRC循环中的索引 */
		for (int i = 0; i < len; i++) /* 传输消息缓冲区 */
		{
			uIndex = (tCRCHi ^ value[i]) & 0x00ff; /* 计算CRC */
	...
	/*
	 * 计算CRC16校验码，返回4位的16进制字符串
	 */
	public static byte[] CheckCRC16(byte[] value, int len) {
		byte[] re = new byte[2];
		CheckCRC16(value, len, re);
		//String ss = String.format("%1$02x", BTI(re[0]));// Integer.toString((re[0]+256)%256,16);
		//ss += String.format("%1$02x", BTI(re[1]));// Integer.toString((re[1]+256)%256,
													// 16);//.toBinaryString(re[1]);
		return re;
	}
