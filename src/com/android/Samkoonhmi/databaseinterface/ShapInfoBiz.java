...
public class ShapInfoBiz extends DataBase {
	...
	public ArrayList<ShapInfo> getShapInfo(SHAP_CLASS shapClass, int sid) {
		...
		cursor = db.getDatabaseBySql(
				"select * from polygon where ePolygonClass =? and nSceneId =?",
				new String[] { shapClassValue + "", sid + "" });
		
		if (null != cursor) {
			while (cursor.moveToNext()) {
				ShapInfo info = new ShapInfo();
				END_POINT_TYPE eCornerType = IntToEnum.getEndPointType(cursor
						.getInt(cursor.getColumnIndex("eCornerType")));
				info.seteCornerType(eCornerType);
				LINE_TYPE eLineType = IntToEnum.getLineType(cursor
						.getInt(cursor.getColumnIndex("eLineType")));
				info.seteLineType(eLineType);
				info.setePolygonClass(shapClass);
				info.setId(cursor.getInt(cursor.getColumnIndex("nItemId")));
				CSS_TYPE eStyle = IntToEnum.getCssType(cursor.getInt(cursor
						.getColumnIndex("eStyle")));
				...
				info.setnForeColor(cursor.getInt(cursor
				...
				info.setnLineWidth(cursor.getInt(cursor
				...
				info.setRoundRectRadiusY(cursor.getInt(cursor
						.getColumnIndex("eCornerType")));
		...
		if (SHAP_CLASS.POLYGON == shapClass) {
			if (id.length() > 0) {
				...
				cursor = db.getDatabaseBySql("select * from point where " + sId
						+ "  and ePointType = 1", null);
				ShapInfo info = null;
				int nItemId = -1;
				int index=0;
				...
				if (null != cursor) {
					while (cursor.moveToNext()) {
						if (nItemId != cursor.getInt(cursor
								.getColumnIndex("nItemId"))) {
							nItemId = cursor.getInt(cursor
									.getColumnIndex("nItemId"));
						...
						int pointX = cursor.getInt(cursor
								.getColumnIndex("nPosX"));
						int pointY = cursor.getInt(cursor
								.getColumnIndex("nPosY"));
