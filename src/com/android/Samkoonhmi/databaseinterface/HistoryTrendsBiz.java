	...
	public ArrayList<HistoryTrendsInfo> select(int sid) {
		...
		if (cursor!=null) {
			while (cursor.moveToNext()) {
				...
				if (null != cursor.getString(cursor.getColumnIndex("bSelectVertMinor")))
				{
					boolean b = cursor.getString(cursor
						.getColumnIndex("bSelectVertMinor")).equals("true")? true : false;
				...
				if (null != cursor.getString(cursor.getColumnIndex("bSelectHorMinor")))
				{
					boolean bb = cursor.getString(cursor
						.getColumnIndex("bSelectHorMinor")).equals("true")? true : false;
				...
				if (null != cursor.getString(cursor.getColumnIndex("bSelectNet")))
				{
					boolean bbb = cursor.getString(cursor
						.getColumnIndex("bSelectNet")).equals("true")? true : false;
...
//				mTrendsInfo.setnHorMinorScale((short) cursor.getInt(cursor.getColumnIndex("nHorMinorScale")));	
				...
				int minute=((mTrendsInfo.getnRecentMonth()*30+mTrendsInfo.getnRecentDay())*24
						+mTrendsInfo.getnRecentHour())*60;
...
//				mTrendsInfo.setnFontAttri((short) cursor.getInt(cursor.getColumnIndex("nFontAttri")));
...
//				mTrendsInfo.setbXmark(cursor.getInt(cursor.getColumnIndex("bXmark"))>0);
				if (null != cursor.getString(cursor.getColumnIndex("bXmark")))
				{
					boolean bbbb = cursor.getString(cursor
						.getColumnIndex("bXmark")).equals("true")? true : false;
				...
				{
					boolean a = cursor.getString(cursor
						.getColumnIndex("bMainVer")).equals("true")? true : false;
					mTrendsInfo.setbMainVer(a);
				}
				if (null != cursor.getString(cursor.getColumnIndex("bMainHor")))
				{
					boolean a = cursor.getString(cursor
						.getColumnIndex("bMainHor")).equals("true")? true : false;
				...
				{
					boolean a = cursor.getString(cursor
						.getColumnIndex("bDate")).equals("true")? true : false;
					mTrendsInfo.setbShowData(a);
				}
				if (null != cursor.getString(cursor.getColumnIndex("bTime")))
				{
					boolean a = cursor.getString(cursor
						.getColumnIndex("bTime")).equals("true")? true : false;
		...
		if (list.size()>0) {
			Cursor result =db.getDatabaseBySql("select * from trendsChannelSet where "+id,null);
			if (null == result) {
...
//					Display.setnDisplayAddr((short) result.getInt(result.getColumnIndex("nDisplayAddr")));
					Channel.setnDisplayAddr(AddrPropBiz.selectById(result.getShort(result
							.getColumnIndex("nDisplayAddr"))));
