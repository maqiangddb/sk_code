...
public class RestorePowerSave {
	...
	public static void restoreLocal()
	{
		...
		int nStartAddr = (nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
		...
		int nAddrLen = (nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
		...
		if(nAddrLen > 0)
		{
			...
			Log.d("PowerSave", "nStartAddr:"+nStartAddr+",nAddrLen:"+nAddrLen);
			if (nStartAddr>-1&&nStartAddr<65536) {
				if (nAddrLen<=1024) {
					PlcRegCmnStcTools.setRegBytesData(mAddr, nLocalB, mSendProp);
				}
			}
		...
		nTmpByte1 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nTmpByte0 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nStartAddr = (nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
		
		nTmpByte3 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nTmpByte2 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nTmpByte1 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nTmpByte0 = nSaveBuff[nStartPos + nIndex++] & 0xff;
		nAddrLen = (nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
		...
		if(nAddrLen > 1)
		{
			...
			Log.d("PowerSave", "nStartAddr:"+nStartAddr+",nAddrLen:"+nAddrLen);
			if (nStartAddr>0&&nStartAddr<65536) {
				if (nAddrLen<=1024) {
					PlcRegCmnStcTools.setRegBytesData(mAddr, nLocalW, mSendProp);
				}
			}
	...
	private static void restoreSave(byte[] nSaveBuff)
	{
		...
		if(nSaveBuff == null || nSaveBuff.length < (nTotalSize - 8)*1028) return ;
		...
		for(int i = 0; i < ((nTotalSize - 8)/10); i++)
		{
			...
			while(nStartPos < (10240*(i+1) - 4))
			{
				...
				long nBytesLen = (nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
				...
				if(nPowSaveType == 1)
				{
					...
					nTmpByte1 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					nTmpByte0 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					long nTime = (nTmpByte7 << 56) + (nTmpByte6 << 48) + (nTmpByte5 << 40) + (nTmpByte4 << 32) + 
							(nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
					...
					while(nValueLen >= 4)
					{
						...
						if(nAddrValueLen > 0 && nAddrValueLen < 20)
						{
							...
							{
								mValue.nValueByteList[n] = nSaveBuff[nStartPos + nIndex++];
							}
						}
				...
				else if(nPowSaveType == 2)
				{
					...
					nTmpByte1 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					nTmpByte0 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					long aTime = (nTmpByte7 << 56) + (nTmpByte6 << 48) + (nTmpByte5 << 40) + (nTmpByte4 << 32) + 
							(nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
					...
					nTmpByte1 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					nTmpByte0 = nSaveBuff[nStartPos + nIndex++] & 0xff;
					long cTime = (nTmpByte7 << 56) + (nTmpByte6 << 48) + (nTmpByte5 << 40) + (nTmpByte4 << 32) + 
							(nTmpByte3 << 24) + (nTmpByte2 << 16) + (nTmpByte1 << 8) + nTmpByte0;
					...
					//Log.d("SKScene", "gid:"+gid+",aid:"+aid+",atime:"+aTime+",ctime:"+cTime);
	 ...
	 */
	private static void restoreDataCollect(Vector<CollectRecordProp > mAllRestorList) 
	{
		if(null == mAllRestorList || mAllRestorList.isEmpty()) return ;
		...
		{
			int nAddrSize = CollectDataInfo.getInstance().getmHistoryInfoList().get(i).getmCollectAddrList().size();
			...
			if(cus != null)
			{
				...
				if(nColumnSize - 2 == nAddrSize)
				{
					...
					if(cus.moveToNext())
					{
						...
						for(int j = 0; j < nAddrSize; j++)
						{
							String sValue = cus.getString(cus.getColumnIndex("data" + j));
	 ...
	 * @return
	 */
	private static boolean bubbleSort(Vector<CollectRecordProp > mCollectRestorList)
	{
		if(null == mCollectRestorList || mCollectRestorList.isEmpty()) return false;
